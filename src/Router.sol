...
contract Router is IRouter {
    ...
    function swapTokenWithV3(ISwapRouter.ExactInputSingleParams memory params) external override {
        ...
        if (diff > 0) TransferHelper.safeTransfer(params.tokenIn, msg.sender, diff);
    }

    function mintV3Position(INonfungiblePositionManager.MintParams calldata params) external override {
        // check mint recipient is sender
        if (params.recipient != msg.sender) revert InvalidRecipient();
        ...
        if (params.amount1Desired > amount1) {
            TransferHelper.safeTransfer(params.token1, msg.sender, params.amount1Desired - amount1);
        }
    }
        ...
        override
    {
        ...
        if (params.amount1Desired > amoun1) {
            TransferHelper.safeTransfer(token1, msg.sender, params.amount1Desired - amoun1);
        }
    }
