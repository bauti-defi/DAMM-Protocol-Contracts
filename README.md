...
### Routers
A Router is an immutable stateless modular contract that interfaces with an external DeFi protocol. It is responsible for verifying and executing trades on behalf of the caller. Routers provide the following garantuees:
...
The `TokenWhitelistRegistry` is a registry of whitelisted tokens. It is responsible for keeping track of the tokens that have been whitelisted by individual investors. Investors can add or remove tokens from the registry at any time. `TokenWhitelistRegistry` callers can **only** whitelist tokens on behalf of themselves.

### [DAMMGnosisSafeModule](./src/base/DAMMGnosisSafeModule.sol)
The `DAMMGnosisSafeModule` is a Gnosis Safe module that allows operators to trade assets escrowed inside the safe on behalf of the Safe owner(s). To enable operators to trade assets on behalf of the Safe owner(s), the Safe owner(s) must first: 
- Enabled the DAMMGnosisSafeModule module on the Gnosis Safe.
...
The DAMMGnosisSafeModule provides the following garantuees:
...
- All Router garantuees are inherited.
