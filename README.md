...
The `DAMMGnosisSafeModule` is a Gnosis Safe module that allows operators to trade assets escrowed inside a safe on behalf of the owner(s). Operators are enabled by the `DAMMGnosisSafeModule` owner. To enable operators to trade assets on their behalf, the Safe owner(s) must first: 
